...
address_tx = b"1Node"  # Transmit address
address_rx = b"2Node"  # Receive address
...
while True:
    if radio.available():
        ...
        print("Device 1 received:", received_message)

        # Stop listening, send response, then return to listening
        radio.stopListening()
        response = "Acknowledged by Device 1"
        radio.write(response.encode('utf-8'))
        print("Device 1 sent:", response)
        radio.startListening()
